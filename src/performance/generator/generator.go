...
func (gen *Generator) init() error {
	...
	pools, err := lib.NewPools(gen.concurrency)
	if err != nil {
		return err
	}
	gen.pools = pools
	fmt.Printf("Initialized. (concurrency=%d)", gen.concurrency)
	return nil
}

/*throttle-定时器通道
  endSign-结束信号通道*/
func (gen *Generator) load(throttle <-chan time.Time, endSign chan<- uint64) {
	callCount := uint64(0)
...
Loop:
	...
	fmt.Printf("Exit load!\n")

}

func (gen *Generator) handleStopSign() {
	gen.cancelSign = 1
	fmt.Println("Closing result channel...")
	close(gen.resultCh)
}

func (gen *Generator) interact(rawReq *lib.RawReq) *lib.RawResp {
	if rawReq == nil {
...
func (gen *Generator) Stop() (uint64, bool) {
	if gen.stopSign == nil {
		return 0, false
	}
...
func (gen *Generator) Status() lib.GenStatus {
	return gen.status
}
